...
export default function App() {
  ...
  const ready = tab === 'pdf' ? !!file && limitsReady && limits.pdf_available !== false : tab === 'website' ? limitsReady && validUrl(url.trim()) : !!text.trim() && !textTooLong;
    ...
    <main id="main">
      ...
      {output === null ? <section className="converter" aria-label="Convert your content">
        ...
        <div className="input-body" id={`panel-${tab}`} role="tabpanel" aria-labelledby={`tab-${tab}`}>
          {tab === 'pdf' && limits.pdf_available === false && <p className="notice warning" role="status">PDF conversion is not enabled on this server. Website and plain text conversion are available.</p>}
