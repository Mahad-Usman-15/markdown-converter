...
Implementation context: React, TypeScript, Vite, and Tailwind CSS on Vercel; FastAPI and its built-in BackgroundTasks on Render. Use CodeMirror for editing and react-markdown with remark-gfm for preview. The interface must accommodate asynchronous jobs and interrupted jobs.
...
The Vercel frontend sends requests directly to the Render API. CORS permits configured frontend origins. FastAPI BackgroundTasks are in-process and can be interrupted; the interface must support that limitation. Do not add a queue dashboard, sign-in UI, pricing, or history navigation to this release.
